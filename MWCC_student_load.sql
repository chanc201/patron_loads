...
UPDATE staging.student_load SET library='MWCC Library';

UPDATE staging.student_load
SET permission_group = 'MWCC Student'
FROM staging.student_load s
LEFT JOIN permission.grp_tree
ON s.permission_group = grp_tree.name AND grp_tree.parent = 19
...
INSERT INTO staging.student_load_log (org_unit, type, count) SELECT 79,'UPDATED', count(usr) FROM staging.student_load WHERE usr IS NOT NULL;
...
INSERT INTO staging.student_load_log (type, count, org_unit, description) SELECT 'not loaded', count(l.ident_value), 79, 'usrname already exists' FROM staging.student_load l JOIN actor.usr u ON l.usrname=u.usrname;
...
INSERT INTO actor.usr_address (valid, within_city_limits, address_type, usr, street1, street2, city, state, country, post_code)
	SELECT TRUE, FALSE, 'home address', s.usr, UPPER(home_street1), UPPER(home_street2), UPPER(home_city), UPPER(home_state), 'USA', home_post_code
	FROM staging.student_load s WHERE home_street1~*'[a-z0-9]' AND do_not_load=FALSE AND usr IS NOT NULL;
INSERT INTO actor.usr_address (valid, within_city_limits, address_type, usr, street1, street2, city, state, country, post_code)
	SELECT TRUE, FALSE, 'local address', s.usr, UPPER(local_street1), UPPER(local_street2), UPPER(local_city), UPPER(local_state), 'USA', local_post_code
	FROM staging.student_load s WHERE local_street1~*'[a-z0-9]' AND do_not_load=FALSE AND usr IS NOT NULL;
...
INSERT INTO staging.student_load_log (org_unit, type, count) SELECT 79,'CREATED', count(usr) FROM staging.student_load WHERE usr IS NOT NULL AND do_not_load=FALSE;
DELETE FROM staging.student_load WHERE usr IS NOT NULL;

COMMIT;


SELECT 1 as seq, created::text, type, count::text, description FROM staging.student_load_log WHERE org_unit=79 AND created>=DATE(NOW())
UNION
SELECT 2 as seq, '----','----','----','-----'
UNION

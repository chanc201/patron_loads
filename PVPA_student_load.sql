 ...
 *
 */
-- To run: psql -f PVPA_student_load.sql -v file=/absolute/path/to/datafile
...
UPDATE staging.student_load SET library='PVPA Library';
UPDATE staging.student_load SET permission_group='PVPA High School' FROM permission.grp_tree gt WHERE student_load.permission_group=gt.name AND gt.parent!=19;
UPDATE staging.student_load SET gender='';
UPDATE staging.student_load SET barcode=barcode WHERE barcode !~*'^[0-9a-z]+$';
...
--Update existing records matching on email
BEGIN;

UPDATE staging.student_load SET usr=u.id FROM actor.usr AS u WHERE student_load.email=u.email;
...
INSERT INTO actor.card (usr, barcode, active) SELECT s.usr, s.barcode, 'TRUE' FROM staging.student_load s LEFT JOIN actor.card c ON s.barcode=c.barcode WHERE s.usr IS NOT NULL AND c.barcode IS NULL;
...
INSERT INTO staging.student_load_log (org_unit, type, count) SELECT 418,'UPDATED', count(usr) FROM staging.student_load WHERE usr IS NOT NULL;
DELETE FROM staging.student_load WHERE usr IS NOT NULL;

COMMIT;

--Create new records when no match on email
BEGIN;

UPDATE staging.student_load SET usrname=usrname WHERE usrname !~*'([0-9|a-z])';
UPDATE staging.student_load SET passwd=UPPER(family_name) WHERE passwd !~*'([a-z|0-9])';
UPDATE staging.student_load SET barcode=barcode WHERE barcode !~*'([0-9|a-z])';
UPDATE staging.student_load SET do_not_load=TRUE FROM actor.usr u WHERE student_load.usrname=u.usrname;
INSERT INTO staging.student_load_log (type, count, org_unit, description) SELECT 'not loaded', count(l.barcode), 418, 'usrname already exists' FROM staging.student_load l JOIN actor.usr u ON l.usrname=u.usrname;
UPDATE staging.student_load SET do_not_load=TRUE FROM actor.card c WHERE student_load.barcode=c.barcode;
INSERT INTO staging.student_load_log (type, count, org_unit, description) SELECT 'not loaded', count(l.barcode), 418, 'barcode already exists' FROM staging.student_load l JOIN actor.usr u ON l.usrname=u.usrname;
...
INSERT INTO actor.card (usr, barcode, active) SELECT s.usr, s.barcode, 'TRUE' FROM staging.student_load s LEFT JOIN actor.card c ON s.barcode=c.barcode WHERE c.id IS NULL AND do_not_load=FALSE;
UPDATE staging.student_load SET card=c.id FROM actor.card c WHERE student_load.usr=c.usr AND c.barcode~'([0-9]$)';
UPDATE staging.student_load SET card=c.id FROM actor.card c WHERE student_load.usr=c.usr AND student_load.card IS NULL;
UPDATE actor.usr SET card=s.card FROM staging.student_load s WHERE usr.id=s.usr AND usr.card IS NULL;
...
INSERT INTO staging.student_load_log (org_unit, type, count) SELECT 418,'CREATED', count(usr) FROM staging.student_load WHERE usr IS NOT NULL AND do_not_load=FALSE;
DELETE FROM staging.student_load WHERE usr IS NOT NULL;

COMMIT;

SELECT 1 as seq, created::text, type, count::text, description FROM staging.student_load_log WHERE org_unit=418 AND created>=DATE(NOW())
UNION
SELECT 2 as seq, '----','----','----','-----'
UNION
...
SELECT 4 as seq, '----','----','----','-----'
UNION
SELECT 5 as seq, barcode, first_given_name, family_name, email FROM staging.student_load
